...
DB_NAME="lxd_db"
DB_HOST="localhost"
API_URL="http://10.184.56.1:8080"   # alamat ip lxd
...
psql_query() {
    # Jalankan query SQL dan ambil hasilnya (single row, single column)
...
case "$RUANG" in
    ...
    *)
        echo "Error: Ruangan tidak terdaftar! Tentukan dengan: f491, f492, f4111, atau f4112"
        exit 1
        ;;
esac
...
if [ "$ACTION" == "start" ]; then
    ...
    if [ -z "$MODULE_ID" ]; then
        echo "Error: modul '$KURSUS' tidak ditemukan di database"
        exit 1
    fi

    ROOM_ID=$(psql_query "SELECT id FROM rooms WHERE nama='$RUANG'")
    if [ -z "$ROOM_ID" ]; then
        echo "Error: ruangan '$RUANG' tidak ditemukan di database"
        exit 1
    fi

    # proses pembuatan sesi baru di database
    COURSE_CODE="${KURSUS}"
    ...
    # Mulai membuat kontainer untuk setiap praktikan
    ...
    for i in $(seq -f "%02g" 1 $JUMLAH_PRAKTIKAN); do
        ...
        # membuat token API untuk kontainer ini, simpan hash ke database
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "  -> GAGAL menyimpan token untuk $NAME ke database"
        fi
        ...
        echo "  -> Snapshot telah dibuat untuk $NAME"
...
# RESET 1 CONTAINER (PER PRAKTIKAN)
    ...
    NAME="$RUANG"   #  diisi nama container misal f491-03
